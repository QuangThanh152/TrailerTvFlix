...
//  trả về những trailer và đoạn giới thiệu trong mãng
const filterVideos = function (videoList) {
    return videoList.filter(({ type, site}) => (type ==="Trailer" || type ==="Teaser") && site === "YouTube");
}
// fetch data
fetchDataFromServer(`https://api.themoviedb.org/3/movie/${movieId}?api_key=${api_key}&append_to_response=casts,videos,images,Creleases&language=en-US`, function(movie) {
    const {
        backdrop_path,
        poster_path,
        title,
    ...
    document.title = `${title} - TodayWatching`;
